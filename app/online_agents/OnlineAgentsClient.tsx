...
interface OnlineAgentsClientProps {
  initialStores: Store[];
}

export default function OnlineAgentsClient({ initialStores }: OnlineAgentsClientProps) {
  const [filter, setFilter] = useState('');

  const stores = initialStores.filter(
    (item) => item.website !== null && item.website !== '',
  );

  const filteredStores = useMemo(
    () => stores.filter((store) => store.name.toLowerCase().includes(filter.toLowerCase())),
    [filter, stores],
  );
      ...
      <div className="w-full py-6">
        ...
        <div className="grid w-full grid-cols-1 gap-8 px-4 py-6 mt-6 border shadow md:grid-cols-2 bg-slate-50 rounded-xl border-slate-100">
          {filteredStores.length === 0 ? (
            <p className="text-base text-zinc-700">No stores found.</p>
          ) : (
            filteredStores.map((store) => {
              const website =
                store.website?.startsWith('https://')
                  ? store.website
                  : `https://${store.website}`;
              ...
              return (
                ...
                >
                  ...
                  <p className="mt-2 text-deep">
                    Website:
                    <span className="pl-3 cursor-pointer text-sky-700">{store.website}</span>
                  </p>
                  <p className="mt-2 text-deep">
